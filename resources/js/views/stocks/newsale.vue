<template>
  <div class="data-padding">
    ...
    <div class="filters">
      ...
      <el-button class="el-button-mobile fr ml-2" type="primary" icon="el-icon-check" @click="saveSale">
        {{ $t('table.save') }}
      </el-button>
            ...
            <el-row :gutter="10">
              <el-col :xs="24" :sm="24" :md="24" :lg="24">
                <div class="total-wrapper text-right">
                  <small class="text-muted">{{ $t('products.totalDiscount') }} ({{ currency }})</small>
                  <h1 class="text-muted">{{ formatedForm.total_discount | currencyFormat }}</h1>
                </div>
                <div class="total-wrapper text-right">
...
export default {
  ...
  computed: {
    ...
    formatedForm() {
      const form = { total: 0, total_discount: 0, profit: 0, items: [] };
      this.form.items.forEach(item => {
        const item_cost = item.unit_price * item.qty;
        const sale_price = item.unit_sale_price * item.qty;
        item.subtotal = sale_price - (sale_price * item.discount / 100);
        item.profit = item.subtotal - item_cost;
  ...
  methods: {
    ...
    handleSelect(product) {
      this.form.items.push({ id: product.id, name: product.name, qty: 1, unit_price: product.unit_price, unit_sale_price: product.unit_sale_price, discount: product.discount });
    },
