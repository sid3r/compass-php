<template>
  <div class="data-padding">
    <div class="filters">
      <el-input v-model="query.keyword" :placeholder="$t('table.keyword')" style="width: 200px;" @keyup.enter.native="handleFilter" />
      <el-select v-model="query.role" :placeholder="$t('table.role')" clearable style="width: 90px" @change="handleFilter">
    ...
    <el-table v-loading="loading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column align="center" label="Actions" width="500">
        <template slot-scope="scope">
          <!-- <router-link v-if="!scope.row.roles.includes('admin')">
          </router-link> -->
          <el-button v-if="!scope.row.roles.includes('admin')" v-permission="['manage users']" type="primary" size="small" icon="el-icon-edit" @click="$router.push('/administrator/users/edit/'+scope.row.id)">
            {{ $t('table.edit') }}
          </el-button>
          <el-button v-if="!scope.row.roles.includes('admin')" v-permission="['manage permissions']" type="warning" size="small" icon="el-icon-edit" @click="handleEditPermissions(scope.row.id);">
            {{ $t('permission.editPermission') }}
          </el-button>
          <el-button v-if="!scope.row.roles.includes('admin')" v-permission="['manage users']" type="danger" size="small" icon="el-icon-delete" @click="handleDelete(scope.row.id, scope.row.name);">
            {{ $t('table.delete') }}
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="dialogPermissionVisible" :title="'Edit Permissions - ' + currentUser.name" :show-close="false" class="el-dialog-primary">
    ...
    <el-dialog :title="'Create new user'" :visible.sync="dialogFormVisible" :show-close="false" class="el-dialog-primary">
